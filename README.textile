...
* HOST_PUBLIC_INSTANCE is the hostname of the instance that you will soon be creating.  run <code>ec2-describe-instances</code> to find this value.  It will look something like <pre>ec2-xxx-xxx-xxx-xxx.compute-x.amazonaws.com</pre>

h4. Getting started...
* launch one of Eric's default images (see "eric Hammond's":http://alestic.com/ page for the most recent ami-images): <code>ec2-run-instances ami-f51aff9c -k amazon-keypair</code> (for a 64-bit image, launch: <code>ec2-run-instances ami-f21aff9b -k amazon-keypair --instance-type m1.large</code>)
